...
- Pour les grands réseaux, le canvas s'agrandit selon les positions réelles et propose `Organiser`, une disposition animée par forces qui espace les cartes et rapproche les acteurs liés, ainsi que `Cadrer` pour ajuster automatiquement le zoom. Après un import d'au moins 20 nœuds, cette disposition démarre automatiquement. Quand les relations sont nombreuses, leurs libellés restent masqués sauf autour du nœud sélectionné.
