...
- Les fichiers techniques `_Opuscule/*.json` et `_Opuscule/*.md` d’un ZIP importé
  restent traités comme manifests de restauration, mais sont aussi copiés dans un
  dossier racine `_Opuscule` visible et en lecture seule dans l’arbre web.
- Le dossier `_Opuscule` est réservé : l’interface ne permet pas de le créer,
  modifier, déplacer, partager ou supprimer. À l’export, ses copies visibles sont
  ignorées et les manifests sont régénérés depuis les données actives.
...
- Pour les grands réseaux, le canvas s'agrandit selon les positions réelles et propose `Organiser`, une disposition animée par forces qui espace généreusement les cartes tout en rapprochant les acteurs liés, ainsi que `Cadrer` pour ajuster automatiquement le zoom en conservant une marge visuelle autour du réseau. Après un import d'au moins 20 nœuds, cette disposition démarre automatiquement. Quand les relations sont nombreuses, leurs libellés restent masqués sauf autour du nœud sélectionné.
- Le bouton `Toutes les dates` désactive le filtre temporel des nœuds et des relations. Dans ce mode, un nœud `position` affiche le titulaire actif pendant l'année civile actuelle.
