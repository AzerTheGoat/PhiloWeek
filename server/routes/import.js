...
router.post('/obsidian', upload.single('vault'), async (req, res) => {
  ...
  try {
      ...
      try {
        ...
        if (existing) {
          ...
          if (conflict === 'overwrite') {
            await db.run("UPDATE files SET content = ?, updated_at = datetime('now') WHERE id = ?", [rawContent, existing.id])
            fileId = existing.id
          } else {
            ...
            await db.run(
              ...
              [fileId, parentId, newName, rawContent]
            )
          }
        } else {
          fileId = uuidv4()
          await db.run(
            "INSERT INTO files (id, parent_id, name, type, content, created_at, updated_at) VALUES (?, ?, ?, 'file', ?, datetime('now'), datetime('now'))",
            [fileId, parentId, fileName, rawContent]
          )
        }

        pathToId[relativePath] = fileId
        await updateTags(db, fileId, rawContent)
        report.imported++
      } catch (err) {
    ...
    // Resolve [[links]]
    const allFiles = await db.all("SELECT id, name FROM files WHERE type = 'file'")
    const nameToId = {}
    allFiles.forEach(f => {
    ...
    for (const file of allFiles) {
      const row = await db.get('SELECT content FROM files WHERE id = ?', [file.id])
      const content = row?.content || ''
      const linkRegex = /\[\[([^\]|]+)(?:\|[^\]]+)?\]\]/g
      let m
      await db.run('DELETE FROM file_links WHERE source_id = ?', [file.id])
